(function($)
{
    ...
    //noinspection FunctionTooLongJS,FunctionNamingConventionJS
    /***
    ...
    function DataView(options)
    {
        ...
        {
            _.forOwn(objectToDelete, function(value, key)
            {
        ...
        function updateIdxById(startingIndex)
        {
            ...
            for (var i = startingIndex, l = items.length; i < l; i++)
            {
                ...
                if (id === undefined)
                {
                    id = getNextId();
                    items[i].id = id;
                    //if the item does not have an id property then we will assign one.
                }
                idxById[id] = i;
            }
        }
        ...
        {
            if (objectIdProperty !== null && objectIdProperty !== undefined)
            {
        ...
        function setPagingOptions(args)
        {
            ...
            if (args.pageNum !== null && args.pageNum !== undefined)
            {
        ...
        function compileAccumulatorLoop(aggregator)
        {
            ...
            var fn = new Function("_items", "for (var " + accumulatorInfo.params[0] + ", _i=0, _il=_items.length; _i<_il; _i++) {" + accumulatorInfo.params[0] +
                " = _items[_i]; " + accumulatorInfo.body + "}");
        ...
        function compileFilter()
        {
            ...
            var tpl = [//"function(_items, _args) { ",
                "var _retval = [], _idx = 0; ", "var $item$, $args$ = _args; ", "_coreloop: ", "for (var _i = 0, _il = _items.length; _i < _il; _i++) { ",
                "$item$ = _items[_i]; ", "$filter$; ", "} ", "return _retval; "
                //"}"
            ].join("");
        ...
        function compileFilterWithCaching()
        {
            ...
            var tpl = [//"function(_items, _args, _cache) { ",
                "var _retval = [], _idx = 0; ", "var $item$, $args$ = _args; ", "_coreloop: ", "for (var _i = 0, _il = _items.length; _i < _il; _i++) { ",
                "$item$ = _items[_i]; ", "if (_cache[_i]) { ", "_retval[_idx++] = $item$; ", "continue _coreloop; ", "} ", "$filter$; ", "} ",
                "return _retval; "
                //"}"
            ].join("");
            tpl = tpl.replace(/\$filter\$/gi, filterBody);
        ...
        function getRowDiffs(rows, newRows)
        {
            ...
            for (var i = from, rl = rows.length; i < to; i++)
            {
                ...
                } else
                {
                    ...
                    //noinspection OverlyComplexBooleanExpressionJS
                    if ((groupingInfos.length && (eitherIsNonData) && item.__group !== r.__group || item.__group && !item.equals(r)) || (eitherIsNonData && // no good way to compare totals since they are arbitrary DTOs
