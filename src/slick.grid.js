...
(function($)
{
    ...
    /* @const */
    var HEADER_ROW_WIDTH_CORRECTION = 2000;
    ...
    function SlickGrid(container, data, columnDefinitions, options)
    {
        ...
        /* @const */
        var tabbingDirections = LU(NAVIGATE_UP, -1, NAVIGATE_DOWN, 1, NAVIGATE_LEFT, -1, NAVIGATE_RIGHT, 1, NAVIGATE_PREV, -1, NAVIGATE_NEXT, 1, NAVIGATE_HOME,
            -1, NAVIGATE_END, 1);
        /* @const */
        var stepFunctions = LU(NAVIGATE_UP, gotoUp, NAVIGATE_DOWN, gotoDown, NAVIGATE_LEFT, gotoLeft, NAVIGATE_RIGHT, gotoRight, NAVIGATE_PREV, gotoPrev,
            NAVIGATE_NEXT, gotoNext, NAVIGATE_HOME, gotoHome, NAVIGATE_END, gotoEnd);
        ...
        function init()
        {
            ...
            $container.empty().addClass("slickgrid-container ui-widget " + uid).attr("role", "grid").attr("tabIndex", 0).attr("hideFocus", "true");
        ...
        function finishInitialization()
        {
            if (initialized < 2)
            {
                ...
                $container.bind("resize.slickgrid", function(e)
                    {
                        resizeCanvas();
                    ...
                    }).bind("focusout.slickgrid", function(e)
                    {
                        ...
                        // }
                    }).fixClick(handleContainerClickEvent, handleContainerDblClickEvent).bind("contextmenu.slickgrid",
                    handleContainerContextMenu).bind("keydown.slickgrid", handleContainerKeyDown);
                $viewport.bind("scroll", handleScrollEvent);
                $headerScroller.bind("contextmenu", handleHeaderContextMenu).fixClick(handleHeaderClick, handleHeaderDblClick).delegate(".slick-header-column",
                    "mouseenter", handleHeaderMouseEnter).delegate(".slick-header-column", "mouseleave", handleHeaderMouseLeave).bind("draginit",
                    handleHeaderDragInit).bind("dragstart", {distance : 3}, handleHeaderDragStart).bind("drag", handleHeaderDrag).bind("dragend",
                    handleHeaderDragEnd);
                ...
                $canvas.bind("keydown", handleKeyDown).fixClick(handleClick, handleDblClick).bind("contextmenu", handleContextMenu).bind("draginit",
                    handleDragInit).bind("dragstart", {distance : 3}, handleDragStart).bind("drag", handleDrag).bind("dragend",
                    handleDragEnd).delegate(".slick-cell", "mouseenter", handleMouseEnter).delegate(".slick-cell", "mouseleave", handleMouseLeave);
        ...
        function updateColumnHeader(columnId, title, toolTip)
        {
            ...
            if (headerNode)
            {
                ...
                $header.attr("title", toolTip).attr("data-title", toolTip).children().eq(0).html(title || "");
        ...
        function createColumnHeaders()
        {
            ...
            $headers.find(".slick-header-column").each(function h_before_headercell_destroy_f()
                {
            ...
            $headerRow.find(".slick-headerrow-column").each(function h_before_headerrowcell_destroy_f()
                {
                    var columnDef = $(this).data("column");
                    if (columnDef)
                    {
            ...
            $footerRow.find(".slick-footerrow-column").each(function h_before_footerrowcell_destroy_f()
                {
                    var columnDef = $(this).data("column");
                    if (columnDef)
                    {
            ...
            function createColumnHeader(columnDef, appendTo, cell)
            {
                ...
                var info = {
                    ...
                    rowspan      : columnDef.headerRowSpan, //cellHeight: cellHeight,
                ...
                info.html = getHeaderFormatter(-2000 + columnDef.headerRow, cell)(-2000 + columnDef.headerRow, cell, columnDef.name, columnDef, null
                    /* rowDataItem */, info);
                ...
                var header = $(stringArray.join("")).data("column", columnDef).appendTo(appendTo);
                return header;
            }
            ...
            function createBaseColumnHeader(columnDef, $appendTo, cell)
            {
                ...
                if (options.showHeaderRow)
                {
                    ...
                    info = {
                        ...
                        colspan      : columnDef.headerColSpan,
                        rowspan      : 1, //cellHeight: cellHeight,
                        //rowMetadata: rowMetadata,
                        //columnMetadata: columnMetadata,
                        columnHeader : {
                            columnDef : columnDef,
                    ...
                    info.html = getHeaderRowFormatter(-1000 + columnDef.headerRow, cell)(-1000 + columnDef.headerRow, cell, columnDef.initialHeaderRowValue,
                        columnDef, null /* rowDataItem */, info);
                    var stringArray = [];
                    stringArray.push("<div");
                    ...
                    headerRowCell = $(stringArray.join("")).data("column", columnDef).appendTo($headerRow);
                ...
                if (options.showFooterRow)
                {
                    ...
                    info.html = getHeaderRowFormatter(-3000 + columnDef.headerRow, cell)(-3000 + columnDef.headerRow, cell, columnDef.initialFooterRowValue,
                        columnDef, null /* rowDataItem */, info);
                    var stringArray = [];
                    stringArray.push("<div");
                    ...
                    footerRowCell = $(stringArray.join("")).data("column", columnDef).appendTo($footerRow);
        ...
        function setupColumnResize()
        {
            ...
            for (i = 0; i < columnCount; i++)
            {
                ...
                if (columnDef.resizable)
                {
                    ...
                    assert(el);
                    $("<div class='slick-resizable-handle' data-column-index='" + i + "' />").appendTo(el)// all touch support here added by KCPT.
        ...
        function createCssRules()
        {
            ...
            var rules = [
                [
                    ".slick-top-panel",
                    "height: " + options.topPanelHeight + "px"
                ],
                [
            ...
            for (i = 0, len = (hasNestedColumns ? nestedColumns.length : 1); i < len; i++)
            {
                rules.push([
                    ".hrt" + i,
                    "top: " + (i * options.headerHeight) + "px"
                ]);
                rules.push([
                    ".hrb" + i,
                    "bottom: " + ((headersCount - i - 1) * options.headerHeight) + "px"
                ]);
            }
            ...
            if (sheet)
            {
                ...
                {
                    addCSSRule(sheet, d[0], d[1], i);
                    /* i could have been -1 here as each rule can be appended at the end */
                });
            } else
            {
        ...
        function destroy()
        {
            ...
            $canvas.unbind();
            $container.empty().removeClass("slickgrid-container ui-widget " + uid).attr("role", null);
        ...
        {
            var i, c, len, widths = [], shrinkLeeway = 0, total = 0, prevTotal, availableWidth = viewportHasVScroll ? viewportW -
                ...
                for (i = 0, len = columns.length; i < len && total < availableWidth; i++)
                {
                    ...
                    } else
                    {
                        growSize = Math.min(Math.floor(growProportion * currentWidth) - currentWidth,
                                (c.maxWidth ? c.maxWidth - currentWidth : 0) || 1000000) || 1;
                    }
                    total += growSize;
                    widths[i] += growSize;
                }
        ...
        function setSortColumns(cols)
        {
            ...
            for (var i = 0, len = sortColumns.length; i < len; i++)
            {
                ...
                if (el)
                {
                    $(el).addClass("slick-header-column-sorted").find(".slick-sort-indicator").addClass(sortCol.sortAsc ? "slick-sort-indicator-asc" : "slick-sort-indicator-desc");
                }
            }
        }
        ...
        function getCellValueAndInfo(row, cell, config)
        {
            ...
            var m = columns[cell], rowDataItem = getDataItem(row);
        ...
        function getFormatter(row, cell)
        {
            ...
            return (columnMetadata && columnMetadata.formatter) ||
                ...
                options.formatterFactory.getFormatter &&
                options.formatterFactory.getFormatter(column, row, cell, rowMetadata, columnMetadata)) ||
        ...
        function getHeaderFormatter(headerRow, cell)
        {
            ...
            // look up by id, then index
            var columnOverrides = rowMetadata && rowMetadata.columns && (rowMetadata.columns[column.id] || rowMetadata.columns[cell]);

            return (columnOverrides && columnOverrides.headerFormatter) ||
                (rowMetadata && rowMetadata.headerFormatter) ||
                column.headerFormatter ||
                (options.formatterFactory &&
                options.formatterFactory.getHeaderFormatter &&
                options.formatterFactory.getHeaderFormatter(column, row, cell, rowMetadata, columnMetadata)) ||
        ...
        function getHeaderRowFormatter(headerRow, cell)
        {
            ...
            return (columnMetadata && columnMetadata.headerRowFormatter) ||
                ...
                (options.formatterFactory &&
                options.formatterFactory.getHeaderRowFormatter &&
                options.formatterFactory.getHeaderRowFormatter(column, row, cell, rowMetadata, columnMetadata)) ||
        ...
        function updateAllDirtyCells(rangeToUpdate, checkIfMustAbort)
        {
            ...
            for (var row = Math.max(rowsCacheStartIndex, rangeToUpdate.top), endrow = Math.min(rowsCache.length - 1, rangeToUpdate.bottom - 1); row <=
            endrow; row++)
            {
                var cacheEntry = rowsCache[row];
                if (cacheEntry)
        ...
        {
            var canvasWidth = $canvas.outerWidth(), canvasHeight = $canvas.height(), hasVScroll = canvasHeight >
                    $viewport.height(), contentWidth = canvasWidth + (hasVScroll ? scrollbarDimensions.width : 0), hasHScroll = contentWidth >
                    $viewport.outerWidth(), contentHeight = canvasHeight + (hasHScroll ? scrollbarDimensions.height : 0);
            return {
                width  : contentWidth,
        ...
        {
            var width = $viewport.outerWidth(), height = $viewport.height(), hasHScroll = $canvas.outerWidth() >
                    width -
                    scrollbarDimensions.width, hasVScroll = $canvas.height() > height - scrollbarDimensions.height;
        ...
        function cleanUpCells(range, row)
        {
            ...
            for (var columnIdx = minCachedCellNodeIndex, end = cellCache.length; columnIdx < end; columnIdx++)
            {
                ...
                if (columnIdx >= columns.length || columnPosLeft[columnIdx] >= range.rightPx || columnPosLeft[columnIdx + colspan] <= range.leftPx)
                {
                    if (row !== activeRow || columnIdx !== activeCell)
                    {
        ...
        {
            var parentNode = $canvas[0], stringArray = [], rows = [], needToReselectCell = false, r, c, l, colspan, dataLength = getDataLength(), aborted = false;
            ...
            if (needToReselectCell && !mandatoryRange)
            {
                ...
                if (focusMustBeReacquired &&
                    focusMustBeReacquired.row ===
                    activeRow &&
                    focusMustBeReacquired.cell ===
                    activeCell &&
        ...
        function handleScroll(dontRenderYet)
        {
            ...
            if (vScrollDist && (vScrollDist > options.rowHeight))
            {
                ...
                } else
                {
                    ...
                    {
                        assert(pageHeight > 0);
                        page = Math.min(numberOfPages - 1,
                            Math.floor(scrollTop * ((virtualTotalHeight - viewportH) / (scrollableHeight - viewportH)) / pageHeight));
                    }
                    pageOffset = Math.round(page * jumpinessCoefficient);
        ...
        function asyncPostProcessRows()
        {
            ...
            out:
                for (; row >= startRow && row <= endRow; row += rowStep)
                {
                    ...
                    for (var columnIdx = cacheEntry.cellNodesByColumnStart, end = Math.min(cellCache.length, columns.length); columnIdx < end; columnIdx++)
                    {
                        var m = columns[columnIdx], node = cellCache[columnIdx];
                        assert(m);
        ...
        function scrollCellIntoView(row, cell, doPaging, doCenteringY)
        {
            ...
            var left = columnPosLeft[cell], right = columnPosLeft[cell + colspan], scrollRight = scrollLeft + viewportW;    // availableWidth = viewportHasVScroll ? viewportW - scrollbarDimensions.width : viewportW
        ...
        function setActiveCellInternal(newCellNode, cfg)
        {
            ...
            {
                makeActiveCellNormal();
                $(activeCellNode).removeClass("active").parent().removeClass("active-row");          // We don't know the old PosY so this only works well for rowspan=1 cells
            }
            ...
            if (newCellNode != null)
            {
                ...
                $(newCellNode).addClass("active");
                if (rowsCache[activePosY] && rowsCache[activePosY].rowNode)
                {
                    $(rowsCache[activePosY].rowNode).addClass("active-row");
                }
        ...
        function makeActiveCellEditable(editor)
        {
            ...
            var info = __extend({}, options.editorOptions, columnDef.editorOptions, rowMetadata && rowMetadata.editorOptions,
                columnMetadata && columnMetadata.editorOptions, {
                    grid           : self,
        ...
        function getSpans(row, cell)
        {
            ...
            var col, colspan, rowspan, metadata, columnData, iRowSpans, iCellSpans, colCount = columns.length, dataLength = getDataLength(), rowI, rowU;
